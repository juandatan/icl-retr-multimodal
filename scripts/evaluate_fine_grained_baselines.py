"""
Evaluate 0-shot and 1-shot CLIP-retrieval baselines for any dataset registered
in src/data/dataset_registry.py (CUB-200-2011, FGVC Aircraft, Oxford-IIIT Pets),
or for Stanford Cars.

These are static benchmarks — run once and store as reference points for
reranker comparison. Results are saved to outputs/evals/ via save_eval_results.

Usage:
    # Image-level split (within-distribution, recommended)
    python scripts/evaluate_fine_grained_baselines.py \
        --dataset cub_200 \
...
ALL_DATASETS = ["stanford_cars"] + list(FINE_GRAINED_DATASETS.keys())
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate 0-shot and 1-shot CLIP-retrieval baselines on a fine-grained dataset"
    )
